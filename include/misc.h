...
#if __cplusplus >= 201103L
template <typename T, T Val> struct integer_constant {
...
template <typename T1, typename T2> void check_type_equal(T1, T2) {
  quark_assert_static((is_same_type<T1, T2>::value), "type not equal!");
}

/// check_type_equal<T>(a)   check if a's Type == T
template <typename T1, typename T2> void check_type_equal(T2) {
  quark_assert_static((is_same_type<T1, T2>::value), "type not equal!");
}

/// check_type_equal<T1,T2>()   check if T1 == T2
template <typename T1, typename T2> void check_type_equal() {
  quark_assert_static((is_same_type<T1, T2>::value), "type not equal!");
}

template <typename T> void check_type_string(const T &) {
...
template <typename T> std::string ToString(T s1) {
  s1->ToString(); // copy
}

template <> inline std::string ToString(bool s1) {
...
template <typename T> T MaxValue(const T) {
  T max_ = ~0;
  T mask_ = ~0;
...
template <typename T> T MinValue(const T) {
  T min_ = 0;
  T mask_ = 0;
...
template <> inline int64_t MinValue(const int64_t) {
  return 0x8000000000000000LL;
}

template <typename T> bool IsMin(const T var) { return var == MinValue(var); }

template <typename T> bool IsMax(const T var) { return var == MaxValue(var); }
